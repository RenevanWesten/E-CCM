...
if __name__ == "__main__":
    ...
    #Basic CC model (optional seed with seed=seed) 
    model       = MOCModel(delta_t=86400 * 50, datafolder='../../Data/E-CCM/Steady_states/')
    model.set_E_A(0.25)
    model.set_noise(0.085)
    
    #Start from the AMOC on branch
    traj_1              = model.run(5000, 100, model.on, stop_transi=False)
    traj_2              = model.run(5000, 100, model.off, stop_transi=False)
    time_run            = model.time + 1
    q_N_1, rho_diff_1   = model.get_q_N(traj_1, density_diff=True)
    q_N_2, rho_diff_2   = model.get_q_N(traj_2, density_diff=True)
    ...
    for real_i in range(len(q_N_1)):
    	...
    	if np.any(q_N_1[real_i] <= 0):
    		#AMOC transition, on to off
    		AMOC_on_to_off.append(real_i)
    
    	if np.any(q_N_2[real_i] >= 10.0):
    		#AMOC transition, off to on
    		AMOC_off_to_on.append(real_i)

    # %%
    fig, ax	= plt.subplots()
    
    ax.fill_between(time_run, np.percentile(q_N_1, 2.5, axis = 0), np.percentile(q_N_1, 97.5, axis = 0), facecolor = 'red', alpha = 0.2)
    ax.fill_between(time_run, np.percentile(q_N_2, 2.5, axis = 0), np.percentile(q_N_2, 97.5, axis = 0), facecolor = 'blue', alpha = 0.2)
    
    ax.plot(time_run, q_N_1[0, 0]+np.zeros(len(time_run)), '--r', linewidth = 2.0, label = 'AMOC on')
    ax.plot(time_run, q_N_2[0, 0]+np.zeros(len(time_run)), '--b', linewidth = 2.0, label = 'AMOC off')
    ...
    for real_i in range(len(AMOC_off_to_on)):
    	if real_i == 0:
    		ax.plot(time_run, q_N_2[AMOC_off_to_on[real_i]], linestyle = '-', color = 'deepskyblue', linewidth = 0.5)
    	if real_i == 1:
    		ax.plot(time_run, q_N_2[AMOC_off_to_on[real_i]], linestyle = '-', color = 'dodgerblue', linewidth = 0.5)
    ...
    ax.set_title('a) AMOC strength (CCM)')

    # %%
    fig, ax	= plt.subplots()
    
    ax.fill_between(time_run, np.percentile(rho_diff_1, 2.5, axis = 0), np.percentile(rho_diff_1, 97.5, axis = 0), facecolor = 'red', alpha = 0.2)
    ax.fill_between(time_run, np.percentile(rho_diff_2, 2.5, axis = 0), np.percentile(rho_diff_2, 97.5, axis = 0), facecolor = 'blue', alpha = 0.2)
    
    ax.plot(time_run, rho_diff_1[0, 0]+np.zeros(len(time_run)), '--r', linewidth = 2.0, label = 'AMOC on')
    ax.plot(time_run, rho_diff_2[0, 0]+np.zeros(len(time_run)), '--b', linewidth = 2.0, label = 'AMOC off')
    
    for real_i in range(len(AMOC_on_to_off)):
    	if real_i == 0:
    		ax.plot(time_run, rho_diff_1[AMOC_on_to_off[real_i]], linestyle = '-', color = 'firebrick', linewidth = 0.5)
    	if real_i == 1:
    		ax.plot(time_run, rho_diff_1[AMOC_on_to_off[real_i]], linestyle = '-', color = 'orangered', linewidth = 0.5)
    
    for real_i in range(len(AMOC_off_to_on)):
    	if real_i == 0:
    		ax.plot(time_run, rho_diff_2[AMOC_off_to_on[real_i]], linestyle = '-', color = 'deepskyblue', linewidth = 0.5)
    	if real_i == 1:
    		ax.plot(time_run, rho_diff_2[AMOC_off_to_on[real_i]], linestyle = '-', color = 'dodgerblue', linewidth = 0.5)
            
    ax.set_xlabel('Model year')
    ax.set_ylabel('Density difference (kg m$^{-3}$)')
    ax.set_xlim(1, 100)
    ax.set_ylim(-2.5, 2.5)
    ax.set_xticks([1,20,40,60,80,100])
    ax.grid()
    ...
    ax.set_title('b) Density difference (CCM)')

    # %%

    #E-CCM with temperature                                      
    model = MOCModel(delta_t=86400 * 20, datafolder='../../Data/E-CCM/Steady_states/', temp_params=0.0000035)   
    model.set_E_A(0.25)
    model.set_noise(0.085)
    ...
    ax.set_title('c) AMOC strength (E-CCM, temperature only)')
    ...
    ax.set_title('d) Density difference (E-CCM, temperature only)')

    # %%

    #E-CCM with temperature and ice parameters   
    model = MOCModel(delta_t=86400 * 20, datafolder='../../Data/E-CCM/Steady_states/', temp_params=0.0000035, ice_params=[20,0.020])   
    model.set_E_A(0.25)
    model.set_noise(0.085)
    ...
    ax.set_title('e) AMOC strength (E-CCM)')
    ...
    ax.set_title('f) Density difference (E-CCM)')
