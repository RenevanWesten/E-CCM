...
class MOCModel():
    ...
    def __init__(self, E_A=0, seed=None, delta_t=86400, datafolder="../../Data/E-CCM/Steady_states/", **opt_params):
        """
...
if __name__ == "__main__":
    ...
    #model = MOCModel(delta_t=86400, datafolder='../../Data/E-CCM/Steady_states/')       

    #E-CCM with temperature                                      
    model = MOCModel(delta_t=86400, datafolder='../../Data/E-CCM/Steady_states/', temp_params=0.0000035)   
    
    #E-CCM with temperature and ice parameters   
    #model = MOCModel(delta_t=86400, datafolder='../../Data/E-CCM/Steady_states/', temp_params=0.0000035, ice_params=[20,0.020])   

    # Set the forcing and initialize the model
    model.set_E_A(0.25)
    model.set_noise(0.10)
    ...
    t0 = time()
    traj = model.run(10, 100, model.on, stop_transi=True, stop_on=False, stop_off=False)
    t1 = time()
    ...
    S_n         = model.get_var('S_n', traj)

    # Get q_N (and density difference) from the trajectories
    q_N, rho_diff = model.get_q_N(traj, density_diff=True)
    
    fig, ax = plt.subplots()
    
    for real_i in range(len(q_N)):
    
        ax.plot(time_run, q_N[real_i] / 10**6.0)
